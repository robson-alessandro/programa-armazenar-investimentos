...
botaoCompra.addEventListener('click',async (event)=>{
    ...
    if(primeiraCompra.checked){
        ...
        await axios.post(`http://localhost:4567/movimentacao/primeira`,compra)
        .then(async ({data}) =>  {
            alert(data)
            await axios.post(`http://localhost:4567/movimentacao`,compra)
...
botaoVenda.addEventListener('click' ,async (event)=>{
    ...
    await axios.post(`http://localhost:4567/movimentacao`,venda)
        .then(({data}) =>  alert(data))
...
botaoDividendo.addEventListener('click',async(evet) =>{
    ...
    await axios.post(`http://localhost:4567/movimentacao/dividendo`,dividendo)
    .then(({data}) => alert(data))
