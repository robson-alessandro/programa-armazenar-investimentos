...
function cadastrarMovimentacao(req ,res){
    conennection.query("INSERT INTO movimentacoes (nome,data_movimentacao,quantidade,valor,tipo) VALUE (?,?,?,?,?)",[req.body.nome, req.body.data, 
        req.body.quantidade, req.body.valor, req.body.tipoMovi],(err)=>{
        if(err){  
...
function cadastrarDividendo(req,res){
    conennection.query("INSERT INTO dividendos (nome,data_dividendo,valor) VALUES (?,?,?)",[req.body.nome, req.body.data, req.body.valor],
    (err)=>{
        if(err){
            res.json(err.message)
        }else{
